...
def nudge_outward(x):
    """
    Avoid numerical issue in grid misc_data by slightly expanding input x.
...
def get_plane_from_flow_data(flow_data, normal_vector="z", x3_value=100):
    """
    Get a plane of misc_data, in form of DataFrame, from a :py:class:`~.FlowData`
    object. This is used to get planes from SOWFA results and FLORIS
    simulations with fixed grids, i.e. curl.

    Args:
        flow_data (np.array): 3D vector field of velocity misc_data. #TODO: is this
    ...
    Returns:
        pandas.DataFrame: Extracted misc_data.
    """
    order = "f"
    if normal_vector == "z":
...
class CutPlane:
    ...
    def __init__(self, df, x1_resolution, x2_resolution, normal_vector):
        ...
        Args:
            df (pandas.DataFrame): Pandas DataFrame of misc_data with
                columns x1, x2, u, v, w.
        """
...
def project_onto(cut_plane_a, cut_plane_b):
    ...
    Args:
        cut_plane_a (:py:class:`~.tools.cut_plane.CutPlane`):
            Plane of misc_data to project from.
        cut_plane_b (:py:class:`~.tools.cut_plane.CutPlane`):
            Plane of misc_data to project onto.
...
#         Args:
#             flow_data (np.array): 3D vector field of velocity misc_data
